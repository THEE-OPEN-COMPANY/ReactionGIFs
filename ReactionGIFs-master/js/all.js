  ...
  Comments = (function (_super) {
        ...
        (function (_this) {
          return function () {
            if (Page.server_info.rev < 160) {
              ...
              Page.cmd("certSelect", [["mojoid.bit", "mojoverse.bit"]]);
            }
            return false;
          };
        })(this)
    ...
    Comments.prototype.applyCommentData = function (elem, comment) {
      ...
      $(".comment-body", elem).html(
        Text.renderMarked(comment.body, {
          sanitize: true,
        })
      );
      $(".user_name", elem)
...
(function () {
  ...
  mojoBlog = (function (_super) {
    ...
    mojoBlog.prototype.addLazyVideos = function () {
    ...
    mojoBlog.prototype.checkLazyVideos = function () {
      var center,
        closest_distance,
        closest_video,
        distance,
        ...
        (function (_this) {
          return function (res) {
            ...
            if (res) {
              ...
              $(".left h2")
                .html(Text.renderMarked(_this.data.description))
                .data("content", _this.data.description);
              return $(".left .links")
                .html(Text.renderMarked(_this.data.links))
    ...
    mojoBlog.prototype.loadLastcomments = function (type, cb) {
      var query;
      if (type == null) {
        type = "show";
      }
      ...
      return false;
      query =
        "SELECT comment.*, json_content.json_id AS content_json_id, keyvalue.value AS cert_user_id, json.directory, post.title AS post_title FROM comment LEFT JOIN json USING (json_id) LEFT JOIN json AS json_content ON (json_content.directory = json.directory AND json_content.file_name='content.json') LEFT JOIN keyvalue ON (keyvalue.json_id = json_content.json_id AND key = 'cert_user_id') LEFT JOIN post ON (comment.post_id = post.post_id) WHERE post.title IS NOT NULL ORDER BY date_added DESC LIMIT 3";
      return this.cmd(
        "dbQuery",
        [query],
        (function (_this) {
    ...
    mojoBlog.prototype.applyLastcommentdata = function (elem, lastcomment) {
      var body, title_hash;
      elem
        .find(".user_name")
      ...
      title_hash = lastcomment.post_title
        .replace(/[#?& ]/g, "+")
        .replace(/[+]+/g, "+");
      return elem
        .find(".postlink")
        .text(lastcomment.post_title)
        .attr(
          "href",
          "?" + this.section + "&Post:" + lastcomment.post_id + ":" + title_hash
        );
    };

    mojoBlog.prototype.applyPagerdata = function (page, limit, has_next) {
      var pager;
      pager = $(".pager");
      if (page > 1) {
        pager
    ...
    mojoBlog.prototype.pagePost = function () {
      ...
      return this.cmd(
        "dbQuery",
        [
          "SELECT *, (SELECT COUNT(*) FROM post_vote WHERE post_vote.post_id = post.post_id) AS votes FROM post WHERE post_id = " +
            this.post_id +
            " LIMIT 1",
        ],
        (function (_this) {
          return function (res) {
            ...
            parse_res = function (res) {
              ...
              if (res.length) {
                ...
                $(".post-full .like")
                  .attr("id", "post_like_" + post.post_id)
                  .on("click", _this.submitPostVote);
            ...
            if (res.error) {
              return _this.cmd(
                "dbQuery",
                [
                  "SELECT *, -1 AS votes FROM post WHERE post_id = " +
                    _this.post_id +
                    " LIMIT 1",
                ],
                parse_res
    ...
    mojoBlog.prototype.pageMain = function (type) {
      var limit, query;
      if (type == null) {
        type = "normal";
      }
      ...
      return this.cmd(
        ...
        (function (_this) {
          return function (res) {
            ...
            if (
              _this.server_info &&
              _this.server_info.rev < 570 &&
              type === "normal"
            ) {
              _this.cmd("wrapperNotification", [
                "error",
                "This site requires mojoNet 0.3.2 rev570 or newer!<br>Please update!",
              ]);
              return;
            }
            parse_res = function (res) {
    ...
    mojoBlog.prototype.checkPublishbar = function () {
      if (
        !this.data["modified"] ||
        this.data["modified"] > this.site_info.content.modified
      ) {
    ...
    mojoBlog.prototype.applyPostdata = function (elem, post, full, delay) {
      ...
      $(".title .editable", elem)
        ...
        .attr(
          "href",
          "?" + this.section + "&Post:" + post.post_id + ":" + title_hash
        )
    ...
    mojoBlog.prototype.submitPostVote = function (e) {
      var elem, inner_path;
      if (!Page.site_info.cert_user_id) {
        Page.cmd("certSelect", [["mojoid.bit", "mojoverse.bit"]]);
        return false;
