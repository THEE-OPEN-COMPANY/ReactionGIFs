...
opener.addheaders = [
    ('User-agent', 'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.41 Safari/537.36')]
        ...
        for row in data["data"]["children"]:
            ...
            if row["data"]["score"] > 4000:
                ...
                if url.endswith(".mp4"):
                    ...
                    video["gif_url"] = url.replace(
                        "www.gfycat.com", "zippy.gfycat.com")+".mp4"
                else:
                    video["gif_url"] = url
                videos.append(video)
        return videos
...
class RedditNsfwGifs(Updater):
    def getPage(self, page=0):
        ...
        data = json.load(opener.open(
            "https://www.reddit.com/r/nsfw_gifs/hot.json"))
        videos = []
        for row in data["data"]["children"]:
            if not row:
                continue
            ...
            if row["data"]["score"] > 200:
                video = {"title": row["data"]["title"],
                         "source": "RedditNsfwGifs"}
                url = row["data"]["url"]
                if url.endswith(".mp4"):
                    video["gif_url"] = url
                elif url.endswith(".gifv"):
    ...
    for video in reversed(videos):
        ...
        if video.get("mp4_url"):
            ...
            try:
                ...
                continue
            mp4_path = "../data/mp4-%s/%s.mp4" % (
                video["source"].lower(), data["next_post_id"])
...
if __name__ == "__main__":
    ...
    if added:
        os.chdir(sys.argv[1])
        os.system("python mojonet.py siteSign  --publish")
